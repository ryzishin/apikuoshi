...
export const UNIFIED_ENDPOINTS = [
  ...
  },
  {
    ...
    d: "One flat episode list (number, title, filler flag when known). v2.7.1: bulk Kitsu fetch (one paginated walk instead of one HTTP call per episode), per-episode TMDB auto-skipped above 50 episodes with ?withThumbs=1 to opt in, and optional ?page=&perPage= pagination with a ready-to-use pagination.next URL.",
    params: [
      { n: "key", d: "same key formats as /api/anime", ex: "frieren" },
      { n: "page", d: "1-indexed page number (only with perPage)", ex: "1" },
      { n: "perPage", d: "page size, 1–200 (omit = return all episodes)", ex: "50" },
      { n: "withThumbs", d: "1 = force per-episode TMDB enrichment even on long shows", ex: "1" },
      { n: "art", d: "0 = skip all provider art, use series-poster fallback", ex: "0" },
    ],
    try: "/api/anime/episodes?key=frieren",
    tip: "For One Piece / Naruto / Conan etc. leave withThumbs off — the bulk Kitsu fetch already fills per-episode titles + thumbnails in one walk. Per-episode TMDB is the part that scales linearly with episode count, so it is gated by EPISODE_TMDB_THRESHOLD (default 50). Response carries totalEpisodes alongside count, plus enrichment.tmdbSkipped when the threshold fired.",
  },
  {
    m: "GET",
    p: "/api/anime/servers",
