...
function withCdnToken(url) {
  ...
  if (!token) return url;
  // v2.5.0 FIX: strip any pre-existing ?token= before appending the fresh
  // one. The CDNs (and our own tokenUpToDate) read the FIRST token param —
  // so a client replaying a URL with an expired token used to stack
  // "...&token=STALE&token=FRESH" and the stale signature kept winning
  // (permanent 403 on proxied replays of older chain/watch responses).
  const stripped = url
    .replace(/([?&])token=[^&]*/g, "$1")
    .replace(/&&+/g, "&")
    .replace(/\?&/, "?")
