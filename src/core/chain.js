...
import { canonicalFor } from "./keys.js";
...
async function stepResolve(input) {
  ...
  if (input.q) {
    ...
    // canonicalFor degrades gracefully: mal-only identities and AniList
    // outages no longer break the chain outright — a listing slug from the
    // degraded identity still drives the whole playback pipeline.
    out.via = "key";
    const resolved = await canonicalFor(input.key);
...
export async function runStreamingChain(input) {
  ...
  return {
    ...
    usage: {
      ...
      watchEndpoint: `/api/watch?key=${encodeURIComponent(canonicalChainKey || "")}&ep=${ep}`,
      episodesEndpoint: `/api/anime/episodes?key=${encodeURIComponent(canonicalChainKey || "")}`,
      serversEndpoint: `/api/anime/servers?key=${encodeURIComponent(canonicalChainKey || "")}&ep=${ep}`,
