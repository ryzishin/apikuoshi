 ...
 * ============================================================
 */
import { kitsuAnimeId, kitsuAnimeImages, kitsuEpisodeData, kitsuAllEpisodes } from "./kitsu.js";
...
/**
 * v2.7.1 — Per-episode TMDB enrichment is auto-skipped when an episode
 * list is longer than this threshold. The bulk Kitsu fetch has no such
 * cap (one bounded-parallel walk handles any length), but TMDB has no
 * bulk endpoint and a per-episode HTTP call would recreate the N+1
 * pattern on long-running shows. Set EPISODE_TMDB_THRESHOLD=0 to
 * disable per-episode TMDB entirely; a very large number to always
 * attempt it. `?withThumbs=1` on /api/anime/episodes overrides the cap.
 */
export const EPISODE_TMDB_THRESHOLD =
  Math.max(parseInt(process.env.EPISODE_TMDB_THRESHOLD, 10) || 50, 0);
...
/**
 * v2.4.0: per-response enrichment deadline. List enrichment runs as a
 * bounded pool; if it exceeds this window the response ships with what is
 * already filled (upstream art + whatever providers finished) — gaps stay
 * explicit nulls per the v2.3 art contract. 0 disables the deadline.
 */
export const ENRICH_DEADLINE_MS =
  Math.max(parseInt(process.env.ENRICH_DEADLINE_MS, 10) || 4000, 0);
 ...
 *
 * List mode (`detail: false`) keeps lists fast — v2.4.0 rules:
 *   - items whose art is ALREADY present from upstream skip the providers
 *     entirely (no Kitsu cover hunt for a row that has a poster — a per-
 *     item upstream call for a decorative field was the single biggest
 *     list-endpoint latency cost; cover stays best-effort: filled whenever
 *     a provider runs anyway, else explicit null per the v2.3 contract);
 *   - Kitsu only RESCUES a missing poster;
 *   - TMDB only rescues a still-missing poster (and only when keyed).
 * Detail mode (`detail: true`) runs the FULL chain for the single item a
 * detail response carries: poster → cover → backdrop → banner → logo.
 *
...
export async function enrichAnimeArt(item = {}, { detail = false, skipArt = false } = {}) {
  ...
  // v2.4.0: list mode — art already present from upstream → no provider call
  const needKitsu = detail ? (needPoster || needCover) : needPoster;
  const needTmdb = detail ? (needPoster || needDetailArt || !art.cover) : needPoster;
...
/**
 * Enrich a list of shaped items IN PLACE with bounded concurrency and a
 * wall-clock deadline (ENRICH_DEADLINE_MS, default 4 s). When the deadline
 * fires the response ships immediately — items the pool did not reach keep
 * their upstream art state (poster or explicit null), so the v2.3 null
 * contract always holds. Returns a small coverage report computed from the
 * FINAL item states: { upstream, kitsu, tmdb, none, total }.
 */
export async function enrichItemList(items, { detail = false, skipArt = false } = {}) {
  ...
  if (skipArt || !enrichmentEnabled() || !list.length) {
    ...
    const pool = mapPool(list, async (item) => {
    ...
    // deadline race — res.json() snapshots synchronously, so late pool
    // mutations after the deadline cannot corrupt an already-sent payload
    await (ENRICH_DEADLINE_MS > 0
      ? Promise.race([pool, new Promise((r) => setTimeout(r, ENRICH_DEADLINE_MS))])
      : pool);
  }
  // coverage from final states — truthful even when the deadline fired
  const coverage = { upstream: 0, kitsu: 0, tmdb: 0, none: 0, total: list.length };
  for (const it of list) {
    if (it?.artSource === "upstream") coverage.upstream++;
    else if (it?.artSource === "kitsu") coverage.kitsu++;
    else if (it?.artSource === "tmdb") coverage.tmdb++;
    else coverage.none++;
  }
  return coverage;
}
...
// ------------------------------------------------------- bulk episode thumbs
/**
 * v2.7.1 — THE BULK episode-thumbnail chain for an ENTIRE episode list.
 *
 * Replaces the per-episode `mapPool(shaped, enrichEpisodeThumb)` loop in
 * /api/anime/episodes, which fired one Kitsu HTTP call PER episode (and
 * optionally one TMDB call PER episode) — the N+1 that made One Piece
 * time out at 1100+ episodes.
 *
 * Strategy:
 *   1. Resolve the Kitsu anime ID ONCE (existing malId → Kitsu mapping).
 *   2. Fetch the WHOLE Kitsu episode index in ONE bounded-parallel
 *      paginated walk (kitsuAllEpisodes). One map, 24h cache, zero
 *      per-episode HTTP for the Kitsu side.
 *   3. For every episode in `items`: fold in upstream thumb → Kitsu
 *      map entry → (optionally) TMDB → series-poster fallback → null.
 *   4. TMDB per-episode calls are GATED by `tmdbThreshold`:
 *        - episode count ≤ threshold → per-episode TMDB allowed (existing
 *          behaviour, retained for short series where the cost is bounded)
 *        - episode count  > threshold → per-episode TMDB SKIPPED, the
 *          `tmdbSkipped: true` flag is set on the coverage report.
 *        - `forceTmdb: true` (the `?withThumbs=1` route param) overrides
 *          the threshold and runs per-episode TMDB regardless of length.
 *
 * Mutates `items` IN PLACE (sets `.thumbnail` + `.thumbSource`, and
 * fills `.title` / `.aired` when the provider has data and the existing
 * value is empty or a placeholder). Returns the coverage report:
 *   { upstream, kitsu, tmdb, poster, none, total, tmdbSkipped }
 *
 * `skipArt` short-circuits to series-poster fallback for every episode
 * (mirrors the existing `?art=0` contract — no provider traffic).
 */
export async function enrichEpisodeThumbsBulk(items, {
  kitsuId = null,
  seriesTitle = "",
  seasonHint = null,
  seriesPoster = "",
  ...
  const list = Array.isArray(items) ? items : [];
  const coverage = {
    upstream: 0, kitsu: 0, tmdb: 0, poster: 0, none: 0,
    total: list.length, tmdbSkipped: false,
  };
  if (!list.length) return coverage;

  // Skip-art fast path: poster fallback everywhere, zero provider traffic.
  if (skipArt || !enrichmentEnabled()) {
  ...
  // ---- 1. resolve Kitsu ID once -----------------------------------------
  let resolvedKitsuId = kitsuId;
  if (!resolvedKitsuId && (seriesTitle || seriesTitle === "")) {
    // resolveKitsuId is best-effort; failure → null → skip Kitsu entirely.
    resolvedKitsuId = await resolveKitsuId({ title: seriesTitle }).catch(() => null);
  }

  // ---- 2. bulk Kitsu episode map (one walk, not N requests) -------------
  let kitsuMap = {};
  if (resolvedKitsuId) {
    kitsuMap = await guard(kitsuAllEpisodes(resolvedKitsuId), `kitsuAllEpisodes(${resolvedKitsuId})`) || {};
  }

  // ---- 3. decide TMDB eligibility ---------------------------------------
  const tmdbAllowed = tmdbEnabled() && seriesTitle &&
    (forceTmdb || list.length <= tmdbThreshold);
  coverage.tmdbSkipped = Boolean(seriesTitle && tmdbEnabled()) && !tmdbAllowed && list.length > tmdbThreshold;

  // ---- 4. fold providers into each episode ------------------------------
  // Per-episode TMDB (when allowed) still needs bounded concurrency —
  // reuse mapPool. Kitsu + upstream + poster are pure in-memory lookups.
  await mapPool(list, async (ep) => {
    if (!ep || ep.number == null) return;
    // (a) upstream thumb already on the record — keep it.
    if (ep.thumbnail) { ep.thumbSource = ep.thumbSource || "upstream"; coverage.upstream++; return; }

    // (b) Kitsu bulk-map lookup — ZERO HTTP, just an object read.
    const k = kitsuMap[ep.number] || null;
    if (k) {
      if (k.thumbnail) { ep.thumbnail = k.thumbnail; ep.thumbSource = "kitsu"; }
      if (k.title && (!ep.title || /^Episode\s*\d+$/i.test(ep.title))) ep.title = k.title;
      if (!ep.titleJapanese && k.titleJapanese) ep.titleJapanese = k.titleJapanese;
      if (!ep.aired && k.aired) ep.aired = k.aired;
      if (ep.thumbnail) { coverage.kitsu++; return; }
    }

    // (c) TMDB per-episode (only when allowed by threshold / forceTmdb).
    if (tmdbAllowed) {
      const tmdb = await guard(
        tmdbEpisodeData(seriesTitle, ep.number, seasonHint, false, ep.aired || null),
        `tmdbEpisodeData(${ep.number})`
      );
      if (tmdb?.result?.thumbnail || tmdb?.result?.aired) {
        ep.thumbnail = tmdb.result.thumbnail ?? ep.thumbnail ?? null;
        if (tmdb.result.thumbnail) ep.thumbSource = "tmdb";
        if (tmdb.result.title && (!ep.title || /^Episode\s*\d+$/i.test(ep.title))) ep.title = tmdb.result.title;
        if (!ep.aired && tmdb.result.aired) ep.aired = tmdb.result.aired;
        if (ep.thumbnail) { coverage.tmdb++; return; }
      }
    }

    // (d) series-poster fallback → (e) explicit null.
    if (seriesPoster) {
      ep.thumbnail = seriesPoster; ep.thumbSource = "poster"; coverage.poster++;
    } else {
      ep.thumbnail = null; ep.thumbSource = null; coverage.none++;
    }
  });
